...
export default function ImagePost(props){
    ...
    const postId = props.post ? props.post._id : props._id;

    function handleStarClick(){
        if(!isLighted){
            setIsLighted(true);
            dispatch(starAPost({user: user, postId: postId})).then(_ => {
                dispatch(getUser({user: user, token: user.token})).then(res => {
                    if(res.payload && res.payload.favoritePostList.includes(postId)){
                        setIsLighted(true);
                    }else
                        setIsLighted(false);
                })
            ...
            setIsLighted(false);
            dispatch(unstarAPost({user: user, postId: postId})).then(_ => {
                dispatch(getUser({user: user, token: user.token})).then(res => {
                    if(res.payload && res.payload.favoritePostList.includes(postId)){
                        setIsLighted(true);
                    }else
    ...
    useEffect(()=>{
        if(user && user.favoritePostList.includes(postId)){
            setIsLighted(true);
        }
    },[user,props])
    ...
    function getUserPost() {
        ...
        if (propUser) {
            userTemp = {
                ...
                profilePictureURL: propUser?.profilePictureURL,
                refresh_token: user?.refresh_token,
                token: user?.token,
                username: propUser?.username,
                _id: propUser?._id,
                isGoogle: propUser?.isGoogle
            }
        }
        ...
        navigate('/profile', { state: userData?.user?._id != data?.user?._id ? data : userData });
    }
        ...
        <div className = {style.contentWrapper}>
            ...
            <div className = {style.content}>
                <div className = {style.header}>
                    <div className = {style.profileImageWrapper} onClick={()=>{getUserPost()}}>
                    ...
                    <div className={style.iconsWrapper}>
                        {
                        location.pathname.includes('profile') && props?.user?._id === user._id ? 
                        <>
